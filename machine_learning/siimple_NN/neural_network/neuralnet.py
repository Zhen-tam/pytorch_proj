...
def writefile(file, y_predict):
    y_predict.reshape(len(y_predict),)
    with open(file, 'w') as outf:
        for i in range(len(y_predict)):
            outf.write(str(int(y_predict[i]))+'\n')
...
if __name__ == '__main__':
    ...
    with open(metrics_out, 'w') as metrics:
        for e in range(num_epoch):
            ...
            for k in range(len(train_labels)):
                X_train = train_data.T[k]
                cache_train = forward_propgation(X_train, parameter)
                y_hat_train = cache_train["y_hat"]
                J_train += loss_function(y_hat_train, y_train[k]) / len(train_labels)
                y_predict_train = np.append(y_predict_train, np.where(y_hat_train == np.max(y_hat_train))[0])
            # print(J_train)

            for j in range(len(test_labels)):
                X_test = test_data.T[j]
                cache_test = forward_propgation(X_test, parameter)
                y_hat_test = cache_test["y_hat"]
                J_test += loss_function(y_hat_test, y_test[j]) / len(test_labels)
                y_predict_test = np.append(y_predict_test, np.where(y_hat_test == np.max(y_hat_test))[0])
            # print(J_test)

            metrics.write("epoch=" + str(e + 1) + " crossentropy(train): "+ str(J_train) + "\n")
            metrics.write("epoch=" + str(e + 1) + " crossentropy(test): " + str(J_test) + "\n")

        y_predict_train.resize(len(train_labels),1)
        y_predict_test.resize(len(test_labels), 1)
    ...
    writefile(train_out, y_predict_train)
    writefile(test_out, y_predict_test)
